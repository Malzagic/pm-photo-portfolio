...
export function AdminCategoriesPage() {
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Delete this category? Warning: Ensure no photos are linked to it.")) return;
  ...
  if (loading) return <div style={{ opacity: 0.5 }}>Loading categories...</div>;
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      <form onSubmit={handleAdd} style={{ display: "flex", gap: 12, marginBottom: "3rem" }}>
        <input
          type="text"
          placeholder="New Category Name (e.g. Street)"
          value={newName}
