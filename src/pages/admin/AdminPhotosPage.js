...
export function AdminPhotosPage() {
    ...
    async function handleDelete(photoId, storagePath) {
        if (!user)
            return;
        const confirmed = window.confirm("Are you sure you want to permanently delete this photo from the server and database?");
        if (!confirmed)
            return;
    ...
    if (loading)
        return <div style={{ opacity: 0.6, padding: "2rem" }}>Loading professional assets...</div>;
    if (error)
        return <div style={{ color: "#ff4444", padding: "2rem" }}>Error: {error}</div>;
    return (<div className="admin-photos-container">
      ...
      <header style={{
        ...
        <Link to="/admin/photos/create" style={{
            padding: "10px 20px",
            background: "#fff",
            color: "#000",
            textDecoration: "none",
      ...
      {photos.length === 0 ? (<p style={{ opacity: 0.5, textAlign: "center", marginTop: "4rem" }}>
          Your portfolio is currently empty. Start by adding your first masterpiece.
        </p>) : (<div style={{ display: "grid", gap: 12 }}>
          {photos.map(photo => (<div key={photo.id} style={{
                    display: "grid",
              ...
              <div style={{ width: 100, height: 60, borderRadius: 4, overflow: "hidden", background: "#111" }}>
                {urls[photo.id] ? (<img src={urls[photo.id]} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }}/>) : (<div style={{ width: "100%", height: "100%", background: "#222" }}/>)}
              </div>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <Link to={`/admin/photos/${photo.id}`} style={{
                    color: "#fff",
                    textDecoration: "none",
                    fontSize: "0.9rem",
                    opacity: 0.6,
                ...
                <button onClick={() => handleDelete(photo.id, photo.storagePath)} disabled={actionLoading === photo.id} style={{
                    background: "none",
                    border: "none",
                    color: "#ff4444",
                    cursor: "pointer",
